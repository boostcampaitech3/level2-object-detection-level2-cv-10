...
data = dict(
    ...
    train=dict(
        ...
        ann_file='/opt/ml/detection/dataset/train0.json',
    ...
    val=dict(
        ...
        classes=('General trash', 'Paper', 'Paper pack', 'Metal', 'Glass',
                 'Plastic', 'Styrofoam', 'Plastic bag', 'Battery', 'Clothing'),
        ann_file='/opt/ml/detection/dataset/val0.json',
        img_prefix='/opt/ml/detection/dataset/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
...
checkpoint_config = dict(max_keep_ckpts=4, interval=1)
log_config = dict(
    interval=50,
    hooks=[
        dict(type='TextLoggerHook'),
        dict(type='MlflowLoggerHook', exp_name='0403_cascade_swin_fpn_focal')
    ])
...
work_dir = './work_dirs/0403_cascade_rcnn_swin_l_fpn_focal'
auto_resume = False
gpu_ids = [0]
